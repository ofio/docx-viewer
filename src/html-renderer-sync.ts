...
import {DomType, IDomImage, IDomNumbering, OpenXmlElement, WmlBreak, WmlHyperlink, WmlNoteReference, WmlSymbol, WmlTable, WmlTableCell, WmlTableColumn, WmlTableRow, WmlText} from './document/dom';
...
interface Element_DOM extends HTMLElement {
	is_overflow?: boolean,
}
...
export class HtmlRendererSync {
	...
	// 根据XML对象渲染单个元素
	renderElement(elem: OpenXmlElement, parent?: HTMLElement | Element): Node_DOM {
		let oNode: Node_DOM;
	...
	// 插入子元素，针对后代元素进行溢出检测
	appendChildren(parent: HTMLElement | Element, children: ChildrenType, xml_element?: OpenXmlElement): boolean {
		// 插入元素
		appendChildren(parent, children);
		// 是否溢出标识
		let is_overflow: boolean = false;
		let {is_split, contentElement, pageIndex, elementIndex, checking_overflow, elements} = this.current_section;
		// 判断当前section是否未拆分，是否需要溢出检测
	...
	renderParagraph(elem: WmlParagraph, parent?: HTMLElement | Element) {
		let oParagraph: Element_DOM = createElement("p");
	...
	renderRun(elem: WmlRun, parent?: HTMLElement | Element) {
		...
		const oSpan: Element_DOM = createElement("span");
	...
	renderTable(elem: WmlTable, parent?: HTMLElement | Element) {
		let oTable: Element_DOM = createElement("table");
	...
	renderTableRow(elem: OpenXmlElement, parent?: HTMLElement | Element) {
		let oTableRow: Element_DOM = createElement("tr");
	...
	renderVmlChildElement(elem: VmlElement) {
		...
		for (let child of elem.children) {
			if (child.type == DomType.VmlElement) {
				...
				this.renderElement(child as any, oSvgElement);
			}
		}
